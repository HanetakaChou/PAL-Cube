...
GLSL_COMPILER_PATH := $(LOCAL_PATH)/../thirdparty/PAL/thirdparty/shaderc/bin/glslc.exe
...
$(SPIRV_DIR)/forward_shading_vertex.inl $(SPIRV_DIR)/forward_shading_vertex.d : $(SHADERS_DIR)/forward_shading_vertex.sl
	$(HIDE) $(call host-mkdir,$(SPIRV_DIR))
	$(HIDE) "$(GLSL_COMPILER_PATH)" -std=310es -mfmt=num -fshader-stage=vert $(GLSL_COMPILER_FLAGS) -MD -MF "$(SPIRV_DIR)/forward_shading_vertex.d" -o "$(SPIRV_DIR)/forward_shading_vertex.inl" "$(SHADERS_DIR)/forward_shading_vertex.sl"

$(SPIRV_DIR)/forward_shading_fragment.inl $(SPIRV_DIR)/forward_shading_fragment.d : $(SHADERS_DIR)/forward_shading_fragment.sl
	$(HIDE) $(call host-mkdir,$(SPIRV_DIR))
	$(HIDE) "$(GLSL_COMPILER_PATH)" -std=310es -mfmt=num -fshader-stage=frag $(GLSL_COMPILER_FLAGS) -MD -MF "$(SPIRV_DIR)/forward_shading_fragment.d" -o "$(SPIRV_DIR)/forward_shading_fragment.inl" "$(SHADERS_DIR)/forward_shading_fragment.sl"
